...
case 'creact': {
    ...
    if (!q.includes(",")) {
        return await socket.sendMessage(sender, {
            text: `😒 Please provide the channel link and emoji separated by a comma.

Example:
.creact https://whatsapp.com/channel/120363396379901844/175,🔥`
        });
    }

    try {
        let [link, ...emojiParts] = q.split(",");
        link = link.trim();
        const emoji = emojiParts.join(",").trim();

        if (!link || !emoji) {
            throw new Error("Channel link or emoji is missing");
        }
        ...
        if (!match) {
            throw new Error(
                "Invalid WhatsApp Channel link. Use: https://whatsapp.com/channel/CHANNEL_ID/MESSAGE_ID"
            );
        }
        ...
        // Make sure the connected account is following the channel.
        if (typeof socket.newsletterFollow === "function") {
            try {
                await socket.newsletterFollow(channelJid);
            } catch (_) {
                // Already following / follow not required by this Baileys build.
            }
        }

        // WhatsApp Channel / Newsletter reaction
        if (typeof socket.newsletterReactMessage === "function") {
            ...
            // Fallback for Baileys builds that don't expose
        ...
        console.log(
            `[CREACT] ${emoji} → ${channelJid} / ${messageId}`
        );
        ...
        await socket.sendMessage(sender, {
            text: `❌ Failed to react to the channel message.
